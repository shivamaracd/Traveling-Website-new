...
export class StatusReportComponent implements OnInit {
  ...
  public selectedTrackingNumber: any = ''
  predefinedRange: string = '';
  clientData : any = []

  constructor( private __clientService:ClientService,public __service: ReportService, private ngxLoader: NgxUiLoaderService, private shipment: ShipmentService) { }
  ...
  ngOnInit(): void {
    ...
    this.getDataMISReport({ target: { value: '' } })
  ...
  printPage(tracking_number: any) {
    window.open(`/report/status-report/print/${tracking_number}`, '_blank');
  }
  ...
  onDateChange() {
    ...
    this.__service.getFilterStatusdata(data).subscribe((res: any) => {
      console.log(res)
      $('#myTable4').DataTable().clear();
      $('#myTable4').DataTable().destroy();
      this.data = res.data;
  ...
  exportToExcel(): void {
    ...
    FileSaver.saveAs(blob, `status-report-${new Date().toISOString().slice(0, 10)}.xlsx`);
  }
